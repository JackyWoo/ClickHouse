...
def test_simple(started_cluster):
    ...
    exec_query_compare_result("SELECT sum(id), val FROM t1_d GROUP BY val ORDER BY val")

    exec_query_compare_result("SELECT sum(id) as ids, val, name FROM t1_d GROUP BY val, name HAVING ids > 10000 ORDER BY val, name")

    exec_query_compare_result("SELECT sum(id), substring(val, 1,2) as v FROM t1_d GROUP BY substring(val, 1,2) ORDER BY v")
