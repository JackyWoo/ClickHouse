...
def started_cluster():
    try:
        ...
        node1.query(
            "INSERT INTO t1_d SELECT n, n, n FROM (SELECT number as n FROM system.numbers limit 10)")
...
def test_simple(started_cluster):
    execute_and_compare("SELECT count() FROM t1_d WHERE (b = 0) AND (c = 0)")
    execute_and_compare("SELECT count() FROM t1_d PREWHERE c = 0 WHERE b = 0")
