...
def started_cluster():
    try:
        ...
        node1.query(
            """CREATE TABLE t1 ON CLUSTER test_cluster (A Int64, B Int8, C String) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/t1', '{replica}') ORDER BY A SETTINGS index_granularity=100;"""
        )

        node1.query(
            """CREATE TABLE t1_d ON CLUSTER test_cluster (A Int64, B Int8, C String) ENGINE = Distributed(test_cluster, default, t1, rand());"""
        )
...
def test_query(started_cluster):
    node1.query(
        "INSERT INTO t1_d SELECT number, 0, if(number between 100 and 200, 'x', toString(number)) FROM numbers(300)")

    node1.query("SYSTEM FLUSH DISTRIBUTED t1_d")

    exec_query_compare_result("SELECT count() FROM t1_d WHERE (B = 0) AND (C = 'x')")

    exec_query_compare_result("SELECT count() FROM t1_d PREWHERE C = 'x' WHERE B = 0")
