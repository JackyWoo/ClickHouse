...
#include <Scheduler/Exchange/ExchangeDataSink.h>
...
FragmentPipelines FragmentPipelinesBuilder::build(bool only_analyze)
{
    FragmentPipelines pipelines;
    for (DistributedFragment & distributed_fragment : distributed_fragments)
    {
        auto fragment = distributed_fragment.fragment;
        const auto & data_to = distributed_fragment.data_to;
        ...
        if (!only_analyze)
        {
            ...
            for (size_t i = 0; i < all_addresses.size(); i++)
            {
                ...
                LOG_DEBUG(log, "Fragment {} will actually send data to {}", fragment->getID(), connection->getReadableHostPort());
        ...
        {
            const auto & data_from = distributed_fragment.data_from;
