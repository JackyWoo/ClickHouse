...
void TCPHandler::processOrdinaryQueryWithCoordination()
{
    ...
    else if (query_kind == ClientInfo::QueryKind::INITIAL_QUERY)
    {
        ...
        {
            auto executor
                = state.io.scheduling_state.pipelines.createFragmentPipelinesExecutor(query_context->getCurrentQueryId(), pipeline, state.io.scheduling_state.storage_limits, interactive_delay / 1000);
